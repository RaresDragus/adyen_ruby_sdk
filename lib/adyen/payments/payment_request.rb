...
module Adyen
  ...
  module Payments
    # The fields described below are the generic payment fields you specify when making an /authorise call
    # to the Adyen payment API.
    #
    # @!attribute [rw] additional_amount
    #   @return [Object] A container for the data concerning the additional amount to be authorised.
    ...
    #   @return [Object] This field contains additional data, which may be required for a particular payment request.
    #     The  additionalData object consists of entries, each of which includes the key and value.
    # @!attribute [rw] amount
    #   @return [Object] A container object for the payable amount information for the transaction.
    #     For BIN or card verification requests, its value needs to be 0 (zero).
    # @!attribute [rw] bank_account
    #   @return [Object] The details of the bank account, from which the payment should be made from.
    #     Either bankAccount or card field must be provided in a payment request
    # @!attribute [rw] billing_address
    #   @return [Object] The address where to send the invoice
    # @!attribute [rw] browser_info
    #   @return [Object] A user's browser information
    ...
    # @!attribute [rw] mpi_data
    #   @return [Object] Authentication data produced by an MPI (Mastercard SecureCode or Verified By Visa)
    ...
    #  @return [String] Defines a recurring payment type
    #    Allowed values:
    #      Subscription: A transaction for a fixed or variable amount, which follows a fixed schedule.
    #      CardOnFile: Card details are stored to enable one-click or omnichannel journeys,
    #                  or simply to streamline the checkout process. Any subscription not following a fixed schedule is
    #                  also considered as a card-on-file transaction.
    ...
    #   @return [UUID] Allowing to retry a request multiple times while only performing the action once
    #
    class PaymentRequest < ApiObject
      attributes :additional_amount, :additional_data, :amount, :bank_account, :billing_address, :browser_info,
                 :capture_delay_hours, :card, :date_of_birth, :dcc_quote, :delivery_address, :delivery_date,
                 :delivery_fingerprint, :fraud_offset, :installments, :mcc, :merchant_account,
                 :merchant_order_reference, :metadata, :mpi_data, :order_reference, :recurring,
                 ...
                 :shopper_reference, :social_security_number, :telephone_number, :threeds2_request_data,
                 :idempotency_key

      before_validation :set_merchant_account

      validates :amount, :merchant_account, :reference, presence: true
